...
from dash_extensions.enrich import DashProxy, html, dcc, Input, Output, State, dash_table, ctx
...
app.layout = html.Div(children=[
                    ...
                    dcc.Markdown(children='''# SF System Client'''),
                    html.Br(),
                    dcc.Markdown(children="Connetion not established...", id='connection_status', style=vis),
                    html.Br(),
                    dcc.RadioItems(['Train or experience models', 'Plot signals', 'Explore logged data'], 'Plot signals', id='task_launcher', style=vis),
                    html.Br(),
                    html.Div(children=[
...
@app.callback(Output('bin_description', 'children'),
...
@app.callback(Output('raw_logging_options', 'style'),
...
@app.callback(Output('tf_container', 'style'),
...
@app.callback(Output('message', 'children', allow_duplicate=True),
...
@app.callback(Output('frequency_bar_selection_panel','style', allow_duplicate=True),
...
@app.callback(Output('logging_opts','style'),
...
@app.callback(Output('logging_counter','data', allow_duplicate=True),
...
@app.callback(Output('n_indiv_samples_to_store','data', allow_duplicate=True),
              Input('signal_buffer_update_interval', 'value'),
...
@app.callback(Output('targetnontarget_freqs', 'data'),
...
@app.callback(Output('get_all_channel_data', 'data'),
...
@app.callback(Output('do_fft', 'data'),
              Output('log_fft', 'data'),
              Output('do_bins', 'data'),
...
@app.callback(Output('log_signal', 'data'),
              Input('log_data','value'),
              Input('signal_visualization_options','value'))
...
# @app.callback(Output("logfile_name", 'style'),
#               Input('log_data', 'value'))
...
@app.callback(Output('connection_status', 'style'),
...
def visualize_post_connection_menu(connection_status):
    if connection_status ==  'Connection to the device server established':
        return b_invis, vis, ['Train or experience models', 'Plot signals', 'Explore logged data'], 'Plot signals' #b_invis, vis, vis, vis, vis, vis, vis, vis,
    else:
        return b_vis, vis, ['Explore logged data'], 'Explore logged data' #b_vis, invis, invis, invis, invis, invis, invis, invis, 
    
@app.callback(Output('model_launcher', 'style'), #HERE - FIX ANOMALOUS LAYOUT MODS
              ...
              Output("update_logplots_button", 'style', allow_duplicate=True),
              Output('ads_sampling_opts','style'),
              Output('logplots_upd_interval', 'disabled', allow_duplicate=True),

              Output('logdata_plotting_control_panel', 'style', allow_duplicate=True),
              Output("clear_plots_button", "style"),
              Output('frequency_bar_selection_panel','style'),
...
@app.callback(Output('sampling_stats_info', 'children'),
...
@app.callback(Output('time_info','children'),
...
@app.callback(Output('logged_df', 'data'),
...
@app.callback(Output('signal_processing_step', 'value'),
...
@app.callback([Output(f'{ch}_raw_buffer', 'data', allow_duplicate=True) for ch in signal_chnames]+
              ...
              [Output('logplots_upd_interval','disabled')]+
...
def clear_buffers(n_clicks, data, data_range, data_step, proc_step, button_text):
    if button_text=="Plot selected data...":
        ...
        for i, chunk in enumerate(chunks):
            ...
            dicts.append(chunk)
        return [[] for i in range(len(signal_chnames)*9)]+[dicts]+['Loaded the framed data to a buffer.']+[wholelen, wholelen]+[0, len(dicts), proc_step, False, True]
    else:
        raise PreventUpdate
    
@app.callback(Output('logplots_upd_interval','disabled', allow_duplicate=True),
...
@app.callback(Output("update_logplots_button", 'children'),
              Input('logplots_upd_interval', 'n_intervals'),
              State('logplots_upd_interval', 'max_intervals'),
...
@app.callback(Output('signal_joint_plot_container', 'children'),
